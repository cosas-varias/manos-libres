...
|---|---|---|
| **nodo** | Servidor Node 20 + TypeScript. Mantiene las sesiones de agente, normaliza sus eventos al protocolo y sirve el WebSocket. | Tu máquina |
...
| **canal de despertar** | Push (FCM o UnifiedPush) para avisar cuando la app no está conectada. | servicio externo |

Nada más. No hay base de datos, no hay servicio en la nube nuestro, no hay cuenta.
...
eventos. Eso se hace con el Claude Agent SDK (`@anthropic-ai/claude-agent-sdk`), que es
Claude Code empaquetado como librería: expone el bucle del agente, sus herramientas
integradas, sus hooks y —lo que más nos importa— un *callback de permisos* que podemos
redirigir al teléfono.
...
### D1 · El transporte es un WebSocket con TLS público

El nodo se publica tras un dominio con certificado (Caddy o nginx) y autentica con un token
de dispositivo. Se eligió frente a una VPN porque funciona desde cualquier red sin
configuración en el móvil, y frente a un relay en la nube porque no añade un componente que
mantener. El coste es un puerto abierto: lo asumimos con las medidas de
...
(«editando `protocol.ts`»). El texto íntegro sigue disponible para cuando el usuario mire
...
[decisión abierta](07-decisiones-abiertas.md#4-calidad-de-voz) por si la narración larga
resulta fatigosa.
...
puede reconstruir la botonera tras reconectar, y el nodo puede reenviar el aviso por push
...
Contrato en [`server/src/adapters/types.ts`](../server/src/adapters/types.ts).
...
El SDK sigue la versión del CLI que empaqueta y su superficie de tipos cambia entre
versiones. Se fija exacta en `package.json` y se sube a mano.
