...
|---|---|
| `net/Protocol.kt` | Los frames de `manos-libres/1` en Kotlin. Réplica de `server/src/protocol.ts`. |
| `net/AgentClient.kt` | WebSocket, reconexión con retroceso exponencial, replay por `sinceSeq`. |
| `session/SessionService.kt` | Foreground service: mantiene el canal con la pantalla bloqueada y recibe los push. |
...
| Permiso | Para qué |
|---|---|
| `INTERNET` | El WebSocket contra el nodo. |
